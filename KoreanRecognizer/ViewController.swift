...
//  KoreanRecognizer
//
//  Created by Сергей Горейко on 01.12.2020.
//
...
class ViewController: UIViewController {
    ...
    func setupVision() {
        ...
        guard let visionModel = try? VNCoreMLModel(for: Korean().model) else {fatalError("can not load Vision ML model")}
    ...
    func handleClassification (request:VNRequest, error:Error?) {
        ...
        DispatchQueue.main.async {
            self.letterLabel.text = classifications.first // update the UI with the classification
        }
    ...
    @IBAction func recognizeLetter(_ sender: Any) {
